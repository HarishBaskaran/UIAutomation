...
public class DataProviderCustomReport implements IReporter {
	...
	protected void writeReportTitle(String title) {
		m_out.print("<center><h1>" + title + " on " + getDateAsString() + "</h1></center>");
	}
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outdir) {
		...
		System.out.println("Harish Report - " + fileName + " generated successfully");
	}

	/** Creates summary of the run */
	public void generateReport1(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
		try {
			m_out = createWriter(outdir);
		} catch (IOException e) {
			return;
...
//	}
	...
	private void resultSummary1(ISuite suite, IResultMap tests, String testname, String style, String details) {

		HashMap<String, String> testMap = new HashMap<String, String>();
		HashMap<HashMap<String, String>, ITestNGMethod> resultMap = new HashMap<HashMap<String, String>, ITestNGMethod>();
		...
		for (ITestNGMethod method : getMethodSet(tests, suite)) {
			...
			for (ITestResult result : tests.getAllResults()) {
				...
				if (hasParameters) {
					for (Object p : parameters) {
						testMap.put(Utils.toString(p), className);
					}
				}
			}
			...
			testMap.forEach((k, v) -> {
				...
				temp.put(k, v);
				resultMap.put(temp, method);
			});

			Set<ITestResult> resultSet = tests.getResults(method);
			String testInstanceName = resultSet.toArray(new ITestResult[] {})[0].getTestName();

			Object[] parameters = resultSet.toArray(new ITestResult[] {})[0].getParameters();
			boolean hasParameters = parameters != null && parameters.length > 0;
			if (hasParameters) {
				for (Object p : parameters) {
		...
		resultMap.forEach((k, v) -> {
			k.forEach((r, t) -> {
				...
				System.out.print(i + "- ");
				System.out.print(Rdata[0].substring(1) + " - ");
				System.out.print(Tdata[Tdata.length - 1] + " - ");
				System.out.println(v.getQualifiedName());
...
//		resultSummary1(suite, tests, testname, style, details);
		...
		if (!tests.getAllResults().isEmpty()) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("</td>").append("<td class=\"numi\">").append(resultSet.size()).append("</td>")
						.append("<td>").append(timeConversion(start)).append("</td>").append("<td class=\"numi\">")
						.append(timeConversion(end - start)).append("</td>").append("</tr>");
			}
			if (mq > 0) {
				cq++;
				m_out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				summaryCell(timeConversion(
						(long) ((overview.getEndDate().getTime() - overview.getStartDate().getTime()) / 1000.))
						+ " seconds", true);
