...
import utils.annotations.PRIORITY;
import utils.annotations.TestCase;
...
public class TestNGCustomReportListener implements IReporter {
	...
	private String fileName = "Customized-Report-" + "-" + formatter.format(date) + ".html";

	private String reportTitle = "Customized Report  ";
	...
	protected void generateMethodSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			if (suites.size() >= 1) {
				titleRow(suite.getName(), 6, Integer.toString(testIndex));
			}
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				for (ITestResult testResult : tests.getResults(method)) {
					...
					if (hasThrowable) {
						...
						while (scanner.hasNext()) {
							if (scanner.next().contentEquals("Screenshot:")) {
								Screenshot = scanner.next();
								break;
							}
						}
				...
				int maxLength = (firstLine.length() < 200) ? firstLine.length() : 200;
				...
				String methodName = qualifiedName(method);
				int methodNameLength = methodName.length();

				TestCase jira = method.getMethod().getAnnotation(TestCase.class);
				PRIORITY priority = method.getMethod().getAnnotation(PRIORITY.class);
				...
				buff.append(
						...
						// "<td> <centre>" +
						// methodName.substring(methodNameLength - 7,
						// methodNameLength - 5)
						// + "</centre></td>"
						// for (int i = 0; i < jiraKeys.length; i++)
						// {jiraKeys[i].toString()}

						"<td>" + ((jiraKeys != null || jiraKey != null)
								? (jiraKeys.length < 2) ? jiraKey.toString() : Arrays.toString(jiraKeys)
								: "-") + " : " + (priority != null ? priority.priority().toString() : "-") + "</td>"
								+ "<td><a href=\"#m" + m_methodIndex + "\">" + methodName + " "
								+ (description != null && description.length() > 0 ? "(\"" + description + "\")" : "")
								+ "</a>" + (null == testInstanceName ? "" : "<br>(" + testInstanceName + ")") + "</td>"
								+ "<td class=\"numi\" style=\"text-align:left;padding-right:2em; "
								+ "width: 500px !important;word-wrap: break-work;word-break:break-all;white-space:normal;\">"
								+ firstLine.substring(0, maxLength) + "<br/>"
								+ (null == Screenshot ? ""
										: "<a href=\"" + Screenshot + "\" target=\"_blank\">" + "Screenshot" + "<br/>")
								+ "</td>" + "<td style=\"text-align:right\">" + formatter.format(calendar.getTime())
	...
	private void startResultSummaryTable(String style) {
		...
		writer.println("<tr><th>Class</th>" + "<th>JIRA Id <br/> Priority</th>"
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				int passCount = getMethodSet(overview.getPassedTests(), suite).size();
				qty_pass_m += passCount;
				summaryCell(passCount, Integer.MAX_VALUE);
				int skipCount = getMethodSet(overview.getSkippedTests(), suite).size();
				qty_skip += skipCount;
				summaryCell(skipCount, 0);
				int failCount = getMethodSet(overview.getFailedTests(), suite).size();
				qty_fail += failCount;
				summaryCell(failCount, 0);

				// Write OS and Browser
				int total = passCount - skipCount - failCount;

				if (total == passCount) {
					summaryCell("Pass", true);
					writer.println("</td>");
				} else {
					summaryCell("Fail", true);
					writer.println("</td>");
				}
		...
		int total = qty_pass_m + qty_skip + qty_fail;
		int status = qty_pass_m - qty_skip - qty_fail;
		writer.println("<tr class=\"total\"><td>Total - " + total + "</td>");
		summaryCell(qty_pass_m, Integer.MAX_VALUE);
		summaryCell(qty_skip, 0);
		summaryCell(qty_fail, 0);
		if (status == qty_pass_m) {
			summaryCell("Pass", "background-color:rgb(124,252,0);", true);
		} else {
			summaryCell("Fail", "background-color:rgb(250,128,114);", true);
	...
	private void summaryCell(String v, String Style, boolean isgood) {
		writer.print("<td class=\"numi\" style=" + Style + (isgood ? "" : "_attn") + ">" + v + "</td>");
	}
	...
	protected void writeReportTitle(String title) {
		writer.print("<center><h1>" + title + " on " + getDateAsString() + "</h1></center>");
	}
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println(".passedodd td {background-color: #3F3}");
		out.println(".passedeven td {background-color: #3F3}");
		out.println(".skippedodd td {background-color: #CCC}");
		out.println(".skippedodd td {background-color: #DDD}");
		out.println(".failedodd td,.numi_attn {background-color:rgb(250,128,114);}");
		out.println(".failedeven td,.stripe .numi_attn {background-color:rgb(250,128,114);}");
